...
define(function (require) {
    ...
    .filter('tranScoreRuleType', function () {//转换积分规则积分方式
    ...
    }).filter('tranScoreRuleIsAbleCss', function () {//转换积分规则启用状态的css
        return function (isable) {
            if (isable == 1) {
                return "scoreRuleIsAble-1";
            }
            return "scoreRuleIsAble-0";
        }
        ...
        .filter('difference', function () {//转换兑换规则兑换类型
            return function (a, b, c, d) {
                return (a + b) - (c + d);
            }
        })
        .filter('percentage', function () {//转换兑换规则兑换类型
            return function (a, b) {
                if (b == 0) {
                    return '-';
                } else {
        ...
        .filter('formatDateToHour', function () {//转换积分规则启用状态
    ...
    }).filter('cardUpgradeStatus', function () {
        //1：消费总额  2：积分总额   3：充值金额  4：余额
    ...
    }).filter('cardUpgradeStatus1', function () {
        //1：消费总额  2：积分总额   3：充值金额  4：余额
        return function (status) {
            if (status == 0) {
                return "开启";
